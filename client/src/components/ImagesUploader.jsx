...
const ImagesUploader = ({ addedImages, onChange }) => {

    const [imageLink, setImageLink] = useState('');

    async function addImageByLink(e) {
        e.preventDefault();
        const { data: filename } = await axios.post('/upload/upload-by-link', { link: imageLink });
    ...
    function removeImage(ev, filename) {
        ...
        onChange([...addedImages.filter(image => image !== filename)]);
    }
    ...
    function selectAsMainImage(ev, filename)
    {
        ...
        const newAddedImages = [filename, ...addedImages.filter(image => image !== filename)];

        onChange(newAddedImages);
    }
        ...
        <>
            ...
            <div className='mt-2 grid gap-2 grid-cols-3 md:grid-cols-4 lg:grid-cols-6'>
                {addedImages.length > 0 && addedImages.map(image => (
                    <div className='h-32 flex relative' key={image}>
                        <img className='rounded-2xl w-full object-cover' src={'http://localhost:4000/uploads/' + image} alt="" />

                        <button onClick={ev => removeImage(ev, image)} className='absolute bottom-1 right-1 text-white bg-black opacity-50 p-2 rounded-2xl hover:cursor-pointer'>
                        ...
                        <button onClick={ev => selectAsMainImage(ev, image)} className='absolute bottom-1 left-1 text-white bg-black opacity-50 p-2 rounded-2xl hover:cursor-pointer'>
